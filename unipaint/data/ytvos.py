...
from . import transforms_video as T
...
class YTVOSDataset(Dataset):
    ...
    """
    def __init__(self, img_folder: Path, ann_file: Path, sample_size: int, return_masks: bool, 
                 num_frames: int):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            # Ensure num_frames doesn't exceed the video length
                    # Check if the video length is sufficient
            if vid_len < num_frames:
                # If the video length is less than num_frames, pick another random index
                idx = random.randint(0, self.__len__() - 1)
                continue  # Go back to the start of the loop with the new index
            
            # Randomly select the starting point for the sequence
            start_frame = random.randint(0, vid_len - num_frames)
            # Generate a list of consecutive frame indices
            sample_indx = list(range(start_frame, start_frame + num_frames))
            ...
            masks = torch.cat([target["masks"].unsqueeze(dim=1)]*3, dim=1).to(torch.float)
            sample = dict(pixel_values=imgs, text=exp, masks=masks)
