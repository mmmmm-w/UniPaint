...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        brushnet: Union[BrushNetModel, None] = None,
    ):
        super().__init__()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # align format for brushnet control guidance
        ...
        # Create tensor stating which brushnets to keep
        if (getattr(self, "brushnet", None) != None) and (init_video != None) and (mask_video != None):
            ...
            for i in range(len(timesteps)):
                ...
                brushnet_keep.append(keeps[0] if isinstance(self.brushnet, BrushNetModel) else keeps)

            init_video = rearrange(init_video, "b c f h w -> (b f) c h w")
            mask_video = rearrange(mask_video, "b c f h w -> (b f) c h w")
            mask_video=(mask_video.sum(1)[:,None,:,:] < 0).to(init_video.dtype)
            conditioning_latents=self.vae.encode(init_video).latent_dist.sample() * 0.18215
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (getattr(self, "controlnet", None) != None) and (controlnet_images != None):
                    ...
                    controlnet_cond = torch.zeros(controlnet_cond_shape).to(latents.device, latents.dtype)

                    controlnet_conditioning_mask_shape    = list(controlnet_cond.shape)
                    controlnet_conditioning_mask_shape[1] = 1
                    controlnet_conditioning_mask          = torch.zeros(controlnet_conditioning_mask_shape).to(latents.device, latents.dtype)
                ...
                if (getattr(self, "brushnet", None) != None) and (init_video != None) and (mask_video != None):
                    ...
                    brushnet_prompt_embeds = text_embeddings
