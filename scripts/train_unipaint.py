    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #Prepare brushnet input
            mask_generator = StaticRectangularMaskGenerator(mask_l=[0,0.4],
                                                      mask_r=[0,0.4],
                                                      mask_t=[0,0.4],
                                                      mask_b=[0,0.4])
            mask = mask_generator(pixel_values).to(local_rank)
            ...
            with torch.no_grad():
                ...
                #prepare brushnet condition
                masked_pixels = rearrange(masked_pixels, "b f c h w -> (b f) c h w")
                mask = rearrange(mask,  "b f c h w -> (b f) c h w")
                mask=(mask.sum(1)[:,None,:,:] < 0).to(masked_pixels.dtype)
                conditioning_latents=vae.encode(masked_pixels).latent_dist.sample() * 0.18215
            ...
            control_timesteps = timesteps.unsqueeze(dim=1)
            control_timesteps = torch.cat([control_timesteps]*16,dim=1).flatten()
            ...
            if is_main_process and (global_step % validation_steps == 0 or global_step in validation_steps_tuple):
                ...
                for idx, (video_path, prompt) in enumerate(zip(video_paths, unipaint_prompts)):
                    ...
                    save_videos_grid(((frame+1)/2).cpu(), f"{output_dir}/samples/sample-{global_step}/{idx}_masked.gif")
                ...
                if not image_finetune:
                    ...
                    masked_videos = torch.concat(masked_videos)
                    filled_videos = torch.concat(filled_videos)
                    save_path = f"{output_dir}/samples/sample-{global_step}.gif"
                    save_videos_grid(samples, save_path)
                    save_videos_grid(masked_videos, f"{output_dir}/samples/sample-{global_step}_masked.gif")
                    save_videos_grid(filled_videos, f"{output_dir}/samples/sample-{global_step}_filled.gif")
